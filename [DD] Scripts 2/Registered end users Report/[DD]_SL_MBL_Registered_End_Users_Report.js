...
function DD_SL_MBL_End_Users_Report(request, response) {
	try {
		...
		var Customer_ID = request.getParameter('distributorid');
...
function generate_json_content(MBL_Registration_Search_ID, Customer_ID) {
	try {
		...
		filters.push(new nlobjSearchFilter('custrecordmblreg_distributor', null, 'is', Customer_ID));

		var resultsObj = null;
		var start = 0,
		end = 1000;
		resultsObj = nlapiLoadSearch('customrecordmblregistration', MBL_Registration_Search_ID);
		...
		while (true) {
			...
			if (results && results.length > 0) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					for (var j = 0; j < columns.length; j++) {
						if ((!results[i].getText(columns[j])) || (results[i].getText(columns[j]) == '')) {
							eachRow[columns[j].getLabel()] = results[i].getValue(columns[j]);
						} else {
							eachRow[columns[j].getLabel()] = results[i].getText(columns[j]);
						}
				...
				responseObject.Registrationdata = RegistrationArray;
				break;

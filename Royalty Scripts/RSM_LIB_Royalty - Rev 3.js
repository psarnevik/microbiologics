 ...
 * 3.00       17 Oct 2016     parnevik         Updated to included recalc of royalties on validate line (but not create royalty item)
 *
 */
...
function UE_SetLineItemRoyaltyValues() {
	...
	for (var i = 1; i <= nlapiGetLineItemCount('item') ; i++) {
		...
		log('Original RATE::::::'+origRate);
		//if(origRate!='' || origRate)
		if(origRate != '' && origRate && origRate!=null){
 ...
 * @param calcOnly - set to true if we are calculating but not adding royalty items to the order
 * @returns {Void}
 */
function onClickUpdateRoyaltyLine(calcOnly) {
    // Initialize some variables.
    var royalty_total = 0.0;
    var royalty_line_num = 0;
...
//        }
    ...
    else if (royalty_total > 0 && !calcOnly) { // Add a new royalty line item

    	
    	
    	
    	nlapiSetFieldValue('custbody_dd_calculated_royalty', royalty_total, false);
    	nlapiSelectNewLineItem('item');
    ...
    else if(royalty_total > 0 && calcOnly){
    	nlapiSetFieldValue('custbody_dd_calculated_royalty', royalty_total, false);
    }
...
function calcRoyalty(){
	...
	if(context == 'webstore' && nlapiGetLineItemCount('item') > 0){
		onClickUpdateRoyaltyLine(true);
	}
}
